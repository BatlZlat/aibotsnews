...
export default function NewsPage() {
  ...
  // Формируем structured data для всех новостей
  const items = news.map(article => generateArticleStructuredData({
    title: article.title,
    excerpt: article.description,
    featuredImage: '',
    author: 'ИИ Боты',
    publishedAt: article.date || new Date().toISOString(),
    updatedAt: article.date || new Date().toISOString(),
    slug: article.slug
  }));
  const collectionStructuredData = {
    '@context': 'https://schema.org',
    '@type': 'CollectionPage',
    'name': 'Новости ИИ ботов',
    'hasPart': items
  };
